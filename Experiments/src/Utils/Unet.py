...
# ===============================================
#               Continuous sinusoidal time embedding
# ===============================================
# Standard sinusoidal-style time embedding that accepts continuous t
class SinusoidalTimeEmbeddings(nn.Module):
    def __init__(self, dim=128, dim_exp=512, max_period=10000.0):
        super().__init__()
        self.dim = dim
        self.dim_exp = dim_exp
    ...
    def forward(self, t):
        """
        t: shape (B,) or (B, 1), float (e.g. in [0, 1] or [0, T])
        returns: shape (B, dim_exp)
        """
        ...
        return self.time_mlp(emb)  # (B, dim_exp)
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, _, H, W = x.shape
        h = self.group_norm(x)
        h = h.reshape(B, self.dim, H * W).transpose(1, 2)  # [B, C, HW] -> [B, HW, C]
        h, _ = self.mhsa(h, h, h)  # [B, HW, C]
        h = h.transpose(2, 1).view(B, self.dim, H, W)  # [B, C, HW] -> [B, C, H, W]
        return x + h
...
class ResnetBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.block2 = Block(
            dim=dim_out, dim_out=dim_out, dropout_rate=dropout_rate, groups=groups
        )

        # Time embedding -> additive bias (standard DDPM style)
        self.dense = nn.Linear(time_emb_dims, dim_out)
    ...
    def forward(self, x, t):
        ...
        # Add time embedding
        h = h + self.dense(self.act(t))[:, :, None, None]
...
class DownSample(nn.Module):
    ...
    # keep *args for interface compatibility with ResnetBlock in loops
    def forward(self, x, *args):
        return self.downsample(x)
...
class UNet(nn.Module):
    def __init__(
        ...
        base_channels=128,  # Number of base channels
        base_channels_multiples=(1, 2, 4, 8),  # Channel multipliers
        apply_attention=(False, True, True, False),  # Attention per resolution
        dropout_rate=0.1,
        ...
        if len(base_channels_multiples) != len(apply_attention):
            raise Exception(
                "base_channels_multiples and apply_attention must have the same length"
            )

        # Time embedding
        time_emb_dims_exp = base_channels * 4  # Ho et al.
        self.time_embeddings = SinusoidalTimeEmbeddings(
            dim=base_channels, dim_exp=time_emb_dims_exp, max_period=max_time_period
        )

        self.init_conv = nn.Conv2d(input_channels, base_channels, 3, 1, 1)
        ...
        # Final block: simple conv block to output_channels
        self.final_block = Block(in_channels, output_channels, dropout_rate=0.0)

    def forward(self, x, t):
        """
        x: (B, C, H, W)
        t: (B,) or (B, 1), float continuous time
        """
        # Ensure t is 1D float tensor of shape (B,)
        if t.dim() == 2 and t.size(-1) == 1:
            t = t.squeeze(-1)
