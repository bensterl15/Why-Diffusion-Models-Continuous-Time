...
def parse_arguments():
    ...
    parser.add_argument("--batch_sample_size", help="Size of each sample batch", type=int, default=1024)
...
def compute_fraction_mem(training_times, train_images, type_model, config, file_fc,
    ...
    for tau in pbar:
        ...
        for i in range(nsamples):
            ...
            try:
                images_a = torch.load(file_a)[:sample_size]
            except FileNotFoundError:
                print(f"Warning: File not found: {file_a}")
                continue
            
            print(f"Loaded samples from: {file_a}", flush=True)
        ...
        #print(f"Checkpoint {tau}: Fmem = {fraction_mem*100:.2f}% ± {std_frac*100:.2f}% "              f"(95% CI: [{lower*100:.2f}%, {upper*100:.2f}%])")
