...
for (j, checkpoint_id) in enumerate(training_times):
    ...
    os.makedirs(path_save, exist_ok=True)  # Simpler

    path_init = path_save + f'{config.TIMESTEPS}/samples_a_{i}'
    path_gen_file = path_save + f'generated/samples_a_{i}'

    print(path_init)
    print(path_gen_file)

    if os.path.exists(path_init) and os.path.exists(path_gen_file):
        print(f'Skipping checkpoint {checkpoint_id}, batch {i} (files exist)')
        continue
